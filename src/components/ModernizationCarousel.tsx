...
const NextArrow: React.FC<any> = ({ onClick }) => (
  <div
    className="absolute top-1/2 right-[-1rem] sm:right-[-1.5rem] z-10 cursor-pointer p-2 bg-background/80 rounded-full shadow hover:bg-background transition"
    onClick={onClick}
...
const PrevArrow: React.FC<any> = ({ onClick }) => (
  <div
    className="absolute top-1/2 left-[-1rem] sm:left-[-1.5rem] z-10 cursor-pointer p-2 bg-background/80 rounded-full shadow hover:bg-background transition"
    onClick={onClick}
    style={{ transform: "translateY(-50%)" }}
  >
            ...
            {highlights.map((item, idx) => (
              <div key={idx} className="px-3">
                <div className="bg-background border border-border rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300">
                  {/* Make image height responsive */}
                  <div className="w-full aspect-[16/9] sm:aspect-[4/3] md:aspect-[16/9] overflow-hidden">
                    <img
                      src={item.image}
                      alt={item.title}
